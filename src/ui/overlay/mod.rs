...
use crate::canvas::{render, Annotation, Canvas, ToolKind};
...
use selection::{draw_handles, handle_at, resize_rect, SelectionEdit};
...
impl eframe::App for OverlayApp {
    ...
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        if let Some(action) = toolbar::show(ctx, &mut self.canvas, &self.palette) {
            match action {
                toolbar::Action::Save => { let r = self.act(false); self.finish(r, ctx); return; }
                toolbar::Action::Copy => { let r = self.act(true);  self.finish(r, ctx); return; }
...
fn handle_tool_input(app: &mut OverlayApp, response: &egui::Response, ctx: &egui::Context) {
    ...
    if response.dragged() {
        match app.selection_edit {
            ...
            SelectionEdit::None => {
                if let (Some(p), Some(d)) = (pointer, app.draft.as_mut()) {
                    d.extend(pos_from(p));
                }
            }
        }
